...
def test_manual_times(client, app, monkeypatch):
    ...
    resp = client.post(f'/api/session/close/{sid}', json={'comment': 'fermeture test', 'closed_time': '11:45'})
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_delete_non_empty_session_forbidden(client, app):
    ...
    with app.app_context():
        u1 = db.session.query(User).filter_by(email='u1@example.org').first()
        ...
        rep = Repair(created=datetime.utcnow().date(), display_id='20250101-001', category=cat, brand=br,
                     initial_state=st, current_state=st2, otype='Obj', model='M1')
        rep.session_id = sid
        db.session.add(rep)
        db.session.commit()
