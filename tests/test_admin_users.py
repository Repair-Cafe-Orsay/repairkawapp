...
def test_clear_membership_year(login_admin):
    ...
    # Effacement direct non supporté (read-only) – poster sans action ne change rien.
    resp = login_admin.post(
        f"/admin/edit/{uid}", data={"name": "Clear Test", "email": "clear@example.org"}
    )
    assert resp.status_code == 302
    with login_admin.application.app_context():
        u = User.query.filter_by(id=uid).first()
...
def test_duplicate_email_creation(login_admin):
    ...
    login_admin.post(
        "/admin/new",
        data={"name": "Dup One", "email": "dup@example.org", "last_membership": "2025"},
    )
    # tentative doublon
    resp = login_admin.post(
        "/admin/new",
        data={"name": "Dup Two", "email": "dup@example.org", "last_membership": "2025"},
    )
    # devrait ré-afficher le formulaire (200) car user_exists True
    assert resp.status_code == 200
    assert (
        b"already" in resp.data.lower()
        or b"exist" in resp.data.lower()
        or b"existe" in resp.data.lower()
    )
...
def test_invalid_membership_year_ignored(login_admin):
    resp = login_admin.post(
        ...
        data={
            "name": "BadYear",
            "email": "badyear@example.org",
            "last_membership": "abcd",
        },
    )
    assert resp.status_code == 302
    with login_admin.application.app_context():
