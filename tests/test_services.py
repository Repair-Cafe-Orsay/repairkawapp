...
from repairkawapp.models import User, Category, State, CloseStatus, Brand, Repair, SpareStatus
from repairkawapp.services.repair_service import generate_display_id, apply_update, create_repair, get_or_create_brand
from repairkawapp.services.stats_service import parse_period, compute_stats, get_cached_lists
...
def app():
    ...
    with app.app_context():
        ...
        db.session.add_all([
            SpareStatus(id=0, label="Inconnu"),
            SpareStatus(id=1, label="Commandé")
        ])
...
def test_generate_display_id_conflicts(app):
    with app.app_context():
        ...
        form = {
            ...
            'manual_id': '001', 'age': '1', 'name': 'A', 'email': 'a@a', 'phone': '',
            'otype': 'Radio', 'model': 'M', 'sn': 'SN', 'description': 'Desc', 'validated': 'on',
            'value': '1', 'weight': '1', 'year': '2024'
        }
...
def test_stats_service_roundtrip(app):
    with app.app_context():
        ...
        form = {
            'date': date(2025, 8, 28).strftime('%Y-%m-%d'),
            'manual_id': None, 'age': '1', 'name': 'A', 'email': 'a@a', 'phone': '',
            'otype': 'Radio', 'model': 'M', 'sn': 'SN', 'description': 'Desc', 'validated': 'on',
            'value': '1', 'weight': '1', 'year': '2024'
        }
...
def test_add_and_delete_spare(login):
    ...
    with login.application.app_context():
        ...
        db.session.commit()
        rid = r.display_id
    # utiliser service add_spare via endpoints déjà testés ailleurs ? Ici on teste fonctions directes
    with login.application.app_context():
        from flask_login import login_user
        user = User.query.filter_by(email='tech@example.org').first()
        login_user(user)
        sp, log = add_spare(db.session, rid, item='Vis', status_id=1, source='Magasin', note='Remplacer')
        db.session.commit()
