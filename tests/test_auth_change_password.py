...
def app():
    ...
    with app.app_context():
        _db.create_all()
        u = User(name="Test User", email="test@example.org", password=generate_password_hash("oldpass"), seqid=1)
        _db.session.add(u)
        _db.session.commit()
    yield app
...
def test_change_password_success(client, app, login):
    r = client.post('/change_password_logged', json={'old_password':'oldpass','new_password':'newpass123'})
...
def test_change_password_bad_old(client, login):
    r = client.post('/change_password_logged', json={'old_password':'wrong','new_password':'newpass123'})
    assert r.status_code == 403
    assert r.get_json()['error'] == 'bad_old'

def test_change_password_too_short(client, login):
    r = client.post('/change_password_logged', json={'old_password':'oldpass','new_password':'xx'})
