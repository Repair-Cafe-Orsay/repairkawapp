...
def test_update_session_details(client, app):
    ...
    r = client.post(
        f"/api/session/update/{sid}",
        json={"location": "Nouveau Lieu", "comment": "note"},
    )
    assert r.status_code == 200
    with app.app_context():
...
def test_attach_repair_switch_session(client, app):
    """Création d'une réparation, fermeture, nouvelle séance puis rattachement."""
    login(client, "owner@example.org")
    ...
    with app.app_context():
        ...
        logs = [
            entry.content for entry in db.session.query(LogModel).filter_by(repair_id=rep2.id).all()
        ]
