...
def upgrade():
    ...
    if 'location' in cols:
        sessions = conn.execute(sa.text('SELECT DISTINCT location FROM session WHERE location IS NOT NULL AND location <> ""')).fetchall()
        name_to_id = {}
        for (loc,) in sessions:
            res = conn.execute(sa.text('INSERT IGNORE INTO location(name) VALUES (:n)'), {'n': loc})
        ...
        for (loc,) in sessions:
            conn.execute(sa.text('UPDATE session SET location_id=:lid WHERE location=:ln'), {'lid': mapping.get(loc), 'ln': loc})
...
def downgrade():
    ...
    conn.execute(sa.text('UPDATE session s JOIN location l ON s.location_id=l.id SET s.location=l.name'))
    with op.batch_alter_table('session') as batch_op:
